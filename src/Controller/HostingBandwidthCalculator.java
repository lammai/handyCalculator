...
public class HostingBandwidthCalculator {
    ...
    public String convert() {
        if (bandwidth.getValue().matches("[0]+")) {
            ...
            String result = String.format("%f", usage * SizeUnit.unitConvert(monthlyUsage.getUnit(), bandwidth.getUnit()) / TimeUnit.Time.MONTH.toSeconds);
            ...
            String result = String.format("%f", usage * SizeUnit.unitConvert(bandwidth.getUnit(), monthlyUsage.getUnit()) * TimeUnit.Time.MONTH.toSeconds);
            result = result.contains(".") ? result.replaceAll("0*$","").replaceAll("\\.$","") : result;
